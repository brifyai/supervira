/**
 * GEMINI 2.5 FLASH - TTS MODULE
 *
 * Este módulo proporciona funciones para usar Gemini 2.5 Flash para generar
...
/**
 * Función principal para generar audio con Gemini 2.5 Flash TTS
 */
export async function generateAudioWithGemini(request: GeminiTTSRequest): Promise<GeminiTTSResponse> {
  try {
    ...
    console.log(`🔄 Generando audio con Gemini TTS para texto (${text.length} caracteres)...`);
    console.log(`🗣️ Voz: ${voice}, Velocidad: ${speed}, Tono: ${pitch}, Volumen: ${volume}`);

    // Construir el payload para la API de Gemini
    // Nota: Gemini 2.5 Flash usa generateContent para audio también
    const payload = {
      contents: [
        {
          parts: [
    ...
    // Gemini puede devolver audio en diferentes formatos
    ...
    if (data.candidates?.[0]?.content?.parts?.[0]?.inlineData) {
      ...
      duration = inlineData.durationMs ? inlineData.durationMs / 1000 : undefined;
    ...
    console.log(`✅ Audio generado exitosamente (${duration?.toFixed(2)}s, ${tokensUsed} tokens, $${cost?.toFixed(6)})`);
...
export async function generateBatchAudio(
  ...
  console.log(`✅ Batch completado: ${successful}/${texts.length} audios generados exitosamente`);
...
export function isValidVoice(voiceId: string): boolean {
  return voiceId in GEMINI_TTS_VOICES || voiceId === GEMINI_CONFIG.voices.default;
}
...
export function normalizeVoiceSettings(settings: Partial<VoiceSettings>): VoiceSettings {
  return {
    pitch: Math.max(-20, Math.min(20, settings.pitch ?? DEFAULT_VOICE_SETTINGS.pitch)),
    speed: Math.max(0.25, Math.min(4.0, settings.speed ?? DEFAULT_VOICE_SETTINGS.speed)),
    volume: Math.max(-96, Math.min(16, settings.volume ?? DEFAULT_VOICE_SETTINGS.volume)),
