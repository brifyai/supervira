...
export function VoiceSelector({
    ...
    async function loadVoices() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log(
            "[VoiceSelector] Loaded voices with WPM:",
            loadedVoices.map((v: Voice) => ({ name: v.name, wpm: v.wpm })),
          );
        } else {
  ...
  useEffect(() => {
    if (voices.length > 0 && !hasInitialized.current) {
      ...
      if (currentVoice) {
        console.log(
          `[VoiceSelector] ✅ Syncing WPM for ${currentVoice.name}: ${currentVoice.wpm}`,
        );
        onChangeRef.current(currentVoice.id, currentVoice.wpm || 150);
      } else if (voices.length > 0) {
        const firstVoice = voices[0];
        console.log(
          `[VoiceSelector] ✅ Auto-selecting ${firstVoice.name} with WPM: ${firstVoice.wpm}`,
        );
    ...
    <div className="space-y-2">
      ...
      >
        <SelectTrigger className="w-full">
          <SelectValue
            placeholder={loading ? "Cargando voces..." : "Seleccionar voz"}
          />
        </SelectTrigger>
        <SelectContent>
          {voices.map((voice) => (
